...
const res = await fetch(
  ...
  // Every Worker QUERY gets its own request ID. This prevents concurrent
  // suggestion/result searches from resolving the wrong Promise.
  let nextWorkerRequestId = 1;
  const pendingWorkerQueries = new Map();
  ...
  function startWorker() {
    try {
      ...
      worker.onmessage = (e) => {
        ...
        if (type === 'RESULTS') {
          const id = requestId;
          const pending = id != null ? pendingWorkerQueries.get(id) : null;

          if (pending) {
            pendingWorkerQueries.delete(id);
            pending.resolve(e.data.items || []);
            return;
          }

          // Ignore stale/unknown Worker responses. With request IDs, an old
          // response must never be allowed to satisfy a newer query.
          log('Ignoring Worker RESULTS for unknown requestId:', id);
        }
      };

      worker.onerror = (err) => {
        workerReady = false;
        log('Worker error; local fallback will be used when possible.', err);
      };
      ...
      worker = null;
      workerReady = false;
      log('Worker unavailable; local search fallback will be used.');
    }
  }
  startWorker();
  ...
  function queryWorker(q, { limit = 50 } = {}) {
    ...
    return new Promise(resolve => {
      ...
      if (worker && workerReady) {
        ...
        pendingWorkerQueries.set(requestId, { resolve, query });

        try {
          worker.postMessage({ type: 'QUERY', requestId, q: query, limit });
          return;
        } catch (e) {
          pendingWorkerQueries.delete(requestId);
          log('Worker query failed; using local fallback.', e);
        }
      }
      ...
      if (fuse && typeof fuse.search === 'function') {
        try { items = (fuse.search(norm(query), { limit }) || []).map(r => r.item); } catch(_) {}
      }
      if (!items || !items.length) {
        items = fallbackFilter(query).slice(0, limit);
      }
      resolve(items || []);
    });
  }

  // Kept for compatibility with existing callers. It now uses the same
  // request-ID based query path as suggestions and results-page searches.
  function runSearch(q, { limit = 50 } = {}) {
    return queryWorker(q, { limit });
  }
...
function renderResultsList(container, items, q) {
  if (!container) return;
  if (!items || !items.length) {
    container.innerHTML = `<p style="padding:12px;color:#666">No results found.</p>`;
    return;
  }

  const html = items.map(item => {
    const s = (item.snippet || item.content || '').slice(0, 180);
    const t = getType ? getType(item) : { key:'page', label: '' };
    const key = (t.key || 'page');
    const label = (t.label || key).replace(/^\w/, c => c.toUpperCase());
    // compute link — prefer enroll-based students link
    const href = studentHrefForItem(item);
    const jump = hasAnchorFlag && hasAnchorFlag(item) ? `<a href="${item.url}" class="srch-pill" style="margin-left:6px" title="Jump to section">Jump ↪</a>` : '';
      ...
      <div style="display:flex;align-items:flex-start;gap:10px">
        ...
        <div style="flex:1 1 auto;min-width:0">
          <h3 style="margin:0 0 6px 0;font-size:18px">
            <a href="${href}">${highlight(item.title || item.name || '', q)}</a>${jump}
          </h3>
...
function renderSuggestion(items) {
  if (!suggest) return;
  // Avoid suggestions on small screens
  ...
  items = items || [];
  items = sortForLeaveQuery ? sortForLeaveQuery(items, q) : items;
  if (isNameish) items = isNameish(q, { allowSingle: true }) ? sortForNameQuery(items, q) : items;
  ...
  const groups = {};
  items.forEach(it => { const k = (getType && getType(it).key) || 'page'; (groups[k] = groups[k] || []).push(it); });

  // top hit (if strong)
  let topHitHtml = '';
  const first = items[0];
  if (first && (first.title_lc || '').startsWith((q||'').toLowerCase()) && q.length >= 2) {
    const t = getType && getType(first) || {};
    const s = (first.snippet || first.content || '').slice(0, 120);
    const jump = hasAnchorFlag && hasAnchorFlag(first) ? `<span class="srch-pill" title="Jump to section">↪</span>` : '';
    ...
    topHitHtml = `
      <div class="srch-group">
        ...
        <a href="${hrefTop}" class="srch-suggest-row" role="option" data-href="${hrefTop}">
          <span class="srch-badge srch-badge--${t.key || 'page'}">${t.label || 'Result'}</span>
          <span style="flex:1">
  ...
  const htmlGroups = groupsOrder.map(k => {
    ...
    const rows = arr.slice(0,5).map(it => {
      ...
      const jump = hasAnchorFlag && hasAnchorFlag(it) ? `<span class="srch-pill" title="Jump to section">↪</span>` : '';
    ...
    return `<div class="srch-group"><div class="srch-group__title">${label}</div>${rows}</div>`;
  }).join('');

  const footer = `<div class="srch-footer"><a href="search.html?q=${encodeURIComponent(q)}">View all results ↵</a></div>`;
  ...
  // Attach click handlers to suggestion rows
  const rows = Array.from(suggest.querySelectorAll('.srch-suggest-row'));
  rows.forEach(rowEl => {
    // remove old handler if present
    if (rowEl._srch_click_handler) rowEl.removeEventListener('click', rowEl._srch_click_handler);
    const handler = function (e) {
      e.preventDefault();
      let href = rowEl.getAttribute('data-href') || rowEl.getAttribute('href') || '';
      const visible = (rowEl.textContent || '').trim();

      // if link contains an anchor/hash go directly
      if (href && href.indexOf('#') !== -1) {
        window.location.href = href;
        return;
      }

      // canonical map fallback (if present in your app)
      const domMap = window.__canonicalAnchorMapFromDOM || {};
      const canonical = (str) => (str||'').toLowerCase().replace(/[^a-z0-9\s]/gi,' ').replace(/\s+/g,' ').trim();
      ...
      // fallback: use text fragment to highlight
      if (visible) {
        const base = (href && href.split('#')[0]) || 'search.html';
  ...
  async function runResultsPage() {
    if (!isResultsPage || !input || !outEl) return;

    let searchSequence = 0;
    let liveTimer = null;

    async function doResultsSearch(rawQuery, { updateUrl = false } = {}) {
      ...
      try {
        ...
        // A newer search may have been entered while the index was building.
        if (sequence !== searchSequence) return;

        const items = await queryWorker(query, { limit: 200 });

        // Worker responses are request-ID matched, but this additional UI
        // sequence guard prevents an older search from repainting the page
        // after a newer search has already started.
        if (sequence !== searchSequence) return;
        ...
        if (sequence !== searchSequence) return;
        setProgress(0);
        showStatus('Search unavailable: ' + (e && e.message ? e.message : e));
        outEl.innerHTML = '';
        console.error('[search:results]', e);
      }
    }

    // Initial query comes only from the URL. This is the single results-page
    // initializer; there is no second independent URL-search initializer.
    const urlQ = new URLSearchParams(location.search).get('q') || '';
    if (urlQ) {
      input.value = urlQ;
      await doResultsSearch(urlQ);
    } else if (qEl) {
      qEl.textContent = '';
    }

    // Live results while typing on search.html.
    input.addEventListener('input', () => {
      if (liveTimer) clearTimeout(liveTimer);
      liveTimer = setTimeout(() => {
        doResultsSearch(input.value, { updateUrl: false });
      }, 160);
    });

    // Keep Enter behavior simple: the shared search-header handler already
    // navigates to search.html?q=...; on the results page we just perform the
    // search immediately and keep the current URL in sync.
    input.addEventListener('keydown', e => {
      if (e.key === 'Enter') {
        e.preventDefault();
        if (liveTimer) clearTimeout(liveTimer);
        doResultsSearch(input.value, { updateUrl: true });
      }
    });
  }
  ...
  (async function init() {
    try {
      ...
      if (form && input) {
        ...
        input.addEventListener('keydown', e => {
          if (e.key === 'ArrowDown') { e.preventDefault(); moveSel(1); }
            ...
            // On search.html the dedicated results-page initializer owns Enter.
      ...
      // Single results-page initializer
